...
class Citybik {
private:
    std::string api_base = "http://api.citybik.es/v2";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("api.citybik.es"));
...
public:
    ...
    pplx::task<json::value> network_by_id(const std::string& network_id) {
        return make_api_call("/networks/" + utility::conversions::to_utf8string( web::uri::encode_data_string(utility::conversions::to_string_t(network_id))),"GET");
    }
};
